...
def _estimate_soc_labels(df: pd.DataFrame) -> np.ndarray:
    ...
    for _, group in df.groupby("cycle_id", sort=False):
        ...
        time = group["t"].to_numpy(dtype=float)
        ...
        span = max(float(ah[-1]), cap, 1e-6)
        discharged = np.nanmedian(current) > 0
...
def main() -> None:
    ...
    if args.pcoe_dir.exists():
        try:
            metrics["pcoe_holdout"] = evaluate_on_pcoe(args.out, args.pcoe_dir, window=args.window)
