...
class BatteryChargingEnv(gym.Env if gym is not None else object):
    ...
    def step(self, action):
        ...
        self.steps += 1
        reward, reward_terms = self.compute_reward(previous, self.state)
        terminated = bool(
    ...
    def compute_reward(self, s_t, s_tp1) -> tuple[float, dict]:
        ...
        high_v = max(float(s_tp1[2] - self.cfg.V_max), 0.0)
        low_v = max(float(self.cfg.V_min - s_tp1[2]), 0.0)
    ...
    def _world_step(self, state: np.ndarray, safe_current: float) -> np.ndarray:
        ...
        delta_soh = max(float(pred[3]), 0.0)
