...
> 写作状态说明：本文按当前工程状态撰写，可作为本科生 AI 创新赛报告 / 论文初稿。文中结果仅使用已经跑通或已经生成的本地实验产物。根据技术审评，本文已收紧包级仿真和老化奖励的表述边界：Python 多单体结果定义为 independent-cell supervisory prototype，不替代串联包 KCL/KVL 物理仿真；现有 ΔSOH 改善主要由物理应力代理项驱动，Mamba ΔSOH 输出暂作为保守上界接口。SOC `<1.5%` 和 external BatteryML trainer 接入仍未完成；Zenodo 6985321 / 18471156 仅作跨数据集诊断与定性展示。
...
实验主线采用 NASA PCoE 同源 NMC 18650 数据，包含 B0005/B0006/B0007/B0018 主集、ARC-FY08Q4 多温度多倍率子集和 Randomized Battery Usage 动态负载子集。当前系统已完成 W0-W4 主链路：SOC 推断链路可运行，B0018 holdout MAE 为 3.48%；SOH 容量比基线在 NASA holdout 上满足 2% RMSE 阈值；Mamba 世界模型在 B0018 holdout 上实现 1-step 电压 MAE 1.42 mV、20-step rollout 电压 MAE 8.04 mV；SOH-aware ECM 安全层在随机动作投影测试中 100% 满足电压边界。端到端快充评估表明，在同初始条件且双方均充至 80% SOC 的 paired episodes 上，SAC 相比 3A CC-CV 将充电时间由 596.5 s 降至 411.75 s，速度提升 30.97%；单循环 ΔSOH 由 0.001859 降至 0.001536，下降 17.37%；实际过压次数为 0。需要强调的是，当前 ΔSOH 差异主要来自基于电流、电压和温度的物理应力代理公式，而不是 Mamba 老化头的独立预测贡献。

进一步地，本文将单体策略复制到 6-cell / 30-cell independent supervisory prototype，并引入 SOC-spread 主动均衡协调器。在 6-cell paired episode 中，SAC 相比 CC-CV 将最低 SOC cell 到 80% 的时间从 1121 s 降至 668 s，平均 ΔSOH 降低 23.01%，末端 SOC spread 降低 28.00%。该 Python 原型验证的是多单体策略复制与协调逻辑，不声称满足真实 6S1P 串联包的单一路径电流约束。为避免使用来源不明的 Simulink 资产作为定量依据，本文还引入 UPC 36-cell pack WLTP+CC-CV 真实数据集进行包级不一致性分析与主动均衡短仿真。基于真实 UPC 高 spread 初值，Python active buck-boost 数字孪生在 30 min 内将 cell voltage spread 从 622.00 mV 降至 334.00 mV，降幅 46.30%。这些结果表明，本文提出的“世界模型 + 强化学习 + 物理安全层”框架能够在单体快充闭环中兼顾速度、安全与老化，并具备向包级均衡协调扩展的可行性。

关键词：电动汽车快充；电池管理系统；Mamba 世界模型；SAC 强化学习；SOC/SOH 估计；ECM 安全层；主动均衡

![图形摘要 本文系统与当前关键结果总览](paper_figures/fig00_graphical_abstract.png)

图形摘要解读：上半部分展示 W1-W5 的闭环结构，下面四个指标卡片给出当前最能支撑答辩的结果：单体快充速度提升、代理老化损耗下降、实际过压为 0、UPC 初始化的包级 spread 降低。底部 caveat 明确 SOC `<1.5%`、Mamba 老化头和真实电站定量标签仍是未完成边界。
...
图 2 解读：训练主线只从 NASA 同源数据进入 SOC/SOH、世界模型和 RL 评估；LG 只提供 SOC warm-start 权重；UPC 和 Zenodo 只位于 W5 验证/展示侧。该图用于防止评审误解为“跨数据集混训”。
...
技术审评后，ECM 投影加入一阶 SOH-R0 修正：

```text
R0_eff = R0_fresh · (2 - clip(SOH, 0.5, 1.0))
```

因此老化程度较高的 cell 会得到更保守的安全电流边界。该修正不重新标定完整 ECM 参数，只用于提高 L3 安全层对内阻老化的保守性。
...
代码默认值已与上述权重对齐。温度项采用软硬混合风险函数：`T <= 40 deg C` 时不惩罚，40 deg C 到 `T_max` 间按二次项增加，超过 `T_max` 后进入 cliff penalty。当前温度权重仍较低，适合作为安全趋势项；若后续主攻热安全控制，需要重新训练更高温度权重的策略。

老化项采用 hybrid 设计：Mamba 输出 `model_delta_soh`，同时计算基于电流、电压、温度应力的 `aging_proxy_delta_soh`，环境取两者较大值作为单步 SOH 损耗。现有评估显示 Mamba 老化头尚未提供有效正贡献，因此本文把当前 ΔSOH 改善解释为“物理应力代理约束下的策略优化”，而不是“Mamba 已准确预测老化”。

训练参数为 `total_steps=60000`、`max_steps=600`、`buffer_size=20000`、`batch_size=64`。训练后策略保存为 `outputs/sac_policy.zip`。

### 3.6 包级策略复制与主动均衡

在包级扩展中，本文先不直接求解开关级 buck-boost 电路，而构建 Python independent-cell supervisory simulator。单体 SAC / CC-CV / MFCC 策略复制到多个 cell，再由 SOC-spread active balancing coordinator 根据单体不一致性修正局部电流分配。该原型不满足真实串联包“所有串联 cell 电流相等”的 KCL 约束，因此只用于验证策略复制和协调逻辑。该层输出 per-cell current / SOC / voltage 轨迹，可进一步导入 Simulink 30 模组或 UPC 12S3P 数据回放流程做物理 plant 验证。
...
1. Python 6-cell / 30-cell independent supervisory simulator：验证单体策略复制与 SOC-spread 均衡协调。
2. UPC 36-cell pack 真实数据分析：验证真实动态工况下 cell voltage spread 与 balancing semicycle 特征，并进行 active buck-boost 短仿真。

其中 30-cell Python 轨迹仅作为与已有 `batterpack.slx` / `buck_boost_balance.slx` 资产对接的接口烟测，不作为论文定量依据。论文定量包级分析优先采用 UPC 公开数据；真正的 6S/30S 串联包结论必须等 Simulink/Simscape plant 加入 KCL/KVL 约束后再给出。
...
图 3 解读：黑色参考曲线来自按 discharge cycle 独立积分和端点校准得到的 SOC 标签，蓝色曲线为 W1 LSTM 输出。整体趋势可跟随 B0018 老化循环，但在低 SOC 区和跨 cell 迁移处仍存在偏差，因此本文只把 3.48% MAE 作为可用于后续软标签的工程基线，不声称已达到 1.5% 目标。

![图 4 NASA SOH baseline 验证结果](paper_figures/fig07_soh_baseline.png)

图 4 解读：左图展示 SOH 预测与容量比参考标签的一致性，右图展示代表 cell 随 cycle 的 SOH 退化轨迹。该图证明 W1 能给世界模型提供退化状态变量，但该 baseline 使用 NASA `Capacity` 字段，因此它是软标签/一致性基准，不是完全无容量标签的部署模型。

为补充架构创新证据，本文进一步使用 W2 Mamba 世界模型的 frozen encoder 提取序列 embedding，并接 Ridge head 预测 SOH。为了避免标签泄露，提取 embedding 前将输入张量中的 SOH 通道统一置为 1.0。该 ablation 采用 B0005/B0006/B0007 训练、B0018 验证。
...
该结果尚未达到 SOH `<2% RMSE` 的部署目标，但说明世界模型表征对 SOH 估计有帮助，可作为 BatteryML-compatible Mamba-head 的初步对比证据。
...
| B0018 20-step rollout V MAE / p95 | 8.04 / 22.03 mV |
| B0018 100-step replay V MAE / p95 | 22.36 / 85.83 mV |
| B0018 600-step replay V MAE / p95 | 170.56 / 550.20 mV |
| ECM 随机动作投影通过率 | 100% |

![图 5 B0018 20-step Mamba 世界模型开环预测](paper_figures/fig03_world_model_rollout.png)

图 5 解读：世界模型只用起始历史窗口进入 open-loop rollout，随后逐步用自身预测更新状态。20-step 电压误差仍保持在 mV 量级，说明 residual Mamba head 能稳定学习短期电压动力学，是 W3 SAC 环境可用的关键前提。

需要注意，20-step open-loop 指标不能完全代表 600-step SAC 部署时的闭环误差。技术审评后，`BatteryChargingEnv._reset_history()` 已从完全重复的 64 步静态历史改为带微小物理噪声的 no-current 初始化，以降低 Mamba 初始输入的分布外程度。随后在 WSL/Mamba/CUDA 环境下对 B0018 真实 traces 做长 horizon replay：使用真实未来动作，但状态由世界模型自身滚动更新。结果显示 100-step 电压 MAE 为 22.36 mV，600-step 电压 MAE 上升到 170.56 mV。这说明当前世界模型可支撑短期动力学与安全层约束下的策略实验，但不应声称已经具备 600-step 单次无校正长预测能力。

![补充图 20 B0018 世界模型长 horizon replay 漂移](paper_figures/fig20_closed_loop_replay.png)

补充图 20 解读：20-step 仍处于 mV 到十几 mV 级短期预测区间，100-step 开始出现可见漂移，600-step 漂移显著放大。该图把模型基强化学习的核心边界说清楚：本文 W3/W4 结果依赖 ECM 硬安全投影和短期滚动决策，不等于世界模型可以独立替代真实电池 plant 做长时间开环预测。

![图 6 高 SOC 下 ECM 安全动作投影效果](paper_figures/fig05_ecm_safety_projection.png)

图 6 解读：在高 SOC 条件下，未经约束的大电流会推高端电压并触碰上限；ECM 安全层会把候选动作投影到安全电流区间，使预测端电压不超过 `V_max`。这张图对应系统的 L3 硬安全层，不依赖 SAC 自己“学会保守”。

动态负载 Randomized 全量复核已完成。修复 `data.step` loader 后，`RW9/RW11/RW12` 也按统一 `stride=64` 缓存，最终覆盖 28/28 个 RW 文件。raw stress-test 结果显示，PCoE 训练出的 residual Mamba 在 Randomized 全量上 one-step V MAE 为 10.07 mV，20-step rollout V MAE 为 103.43 mV、p95 为 763.48 mV。进一步的数据质量检查发现，`RW2/RW3` 温度列分别有 90.55% / 73.38% 样本为约 `-4094 deg C` 的物理不可能值，且已有文献曾因温度测量损坏排除 `RW3`。因此本文采用可复现的温度 QC 口径：对 `[-40, 120] deg C` 之外的温度插值修复，并将坏温度比例超过 50% 的 cell 从主指标剔除。QC main result 保留 26/28 个 RW 文件，one-step V MAE 为 2.17 mV，20-step rollout V MAE / p95 为 24.29 mV / 92.71 mV。raw 28-file 结果保留为压力测试证据，QC 结果作为正文主指标。

![补充图 19 Randomized 全量 20-step rollout 复核](paper_figures/fig19_randomized_rollout_recheck.png)

补充图 19 解读：左图给出 raw 28 个 RW 文件的逐文件 20-step 电压 MAE，红色标出因温度 QC 被剔除的 `RW2/RW3`；右图对比 raw stress-test 与 QC main result。该图主动揭示数据质量问题和动态负载边界，避免只展示顺滑子集。
...
对现有 `outputs/eval_w4_final_default/trajectories.csv` 进行老化信号分解后，三种策略的 `model_delta_soh` 总和均为 0，`aging_proxy_delta_soh` 主导比例为 100%。因此表中的 ΔSOH 改善应解释为：SAC 在物理应力代理公式约束下学会减少高电流/高电压风险和缩短暴露时间。Mamba 世界模型当前主要提供 SOC/V/T 动力学环境，尚不能作为独立老化预测模型来支撑寿命最优性结论。
...
图 7 解读：episode return 在 reward sweep 后进入上升区间，critic loss 随训练逐渐收敛。它说明当前权重组合能让策略学习到稳定行为，但曲线不是严格单调，因此本文仍保留 Randomized 与奖励泛化复核任务。

![图 8 单体 paired episodes 核心指标对比](paper_figures/fig04_w4_metrics_bar.png)

图 8 解读：该柱状图只统计同一初始条件下 SAC 和 CC-CV 都达到 80% SOC 的 paired episodes，避免把 baseline 未命中目标的样本直接拿来做速度百分比。结果表明 SAC 同时缩短到达时间并降低单循环 ΔSOH，是 W4 最核心的“速度-老化协同”证据。

![图 9 单体 SAC、CC-CV 与 MFCC 充电轨迹对比](paper_figures/fig09_charging_comparison_polished.png)

图 9 解读：四联图展示三种策略的电流、电压、SOC 和温度轨迹。SAC 不只是简单拉高恒流，而是在接近安全边界时通过 ECM 投影和 reward 惩罚形成动态电流曲线，因此能更快到 80% 且实际过压为 0。

该结果满足 W4 核心目标：充电速度提升 ≥ 15%、ΔSOH 降低 ≥ 10%、过压 = 0。

### 5.4 多单体策略复制结果

Python 6-cell independent supervisory prototype 采用更严格的停止口径：最低 SOC cell 达到 80%。在 3 个 episode 中，SAC hit_rate 为 3/3，CC-CV 为 1/3，MFCC 为 0/3。该原型中的 per-cell current 可独立变化，因此不能被称作物理 6S1P 串联包仿真；它验证的是单体策略复制、弱单体约束和 SOC-spread 协调逻辑。
...
在双方都命中的 paired episode 上，SAC 相比 CC-CV 将最低 SOC cell 到 80% 的时间从 1121 s 降至 668 s，平均 ΔSOH 降低 23.01%，末端 SOC spread 降低 28.00%。

![图 10 6-cell 多单体策略复制与均衡协调结果](paper_figures/fig10_pack_comparison_polished.png)

图 10 解读：停止条件采用更严格的最低 SOC cell 到达 80%，因此它比单体 W4 更接近真实 BMS 的“最弱单体约束”。SAC 复制到多个 cell 后仍能提高 hit rate，并在 SOC-spread coordinator 作用下降低末端不一致性。该图不是串联包电路仿真，后续需要 Simulink/Simscape 或 liionpack/PyBaMM 类 plant 约束串联电流后再声称 6S/30S 包级性能。
...
图 11 解读：该图是本项目重画的主动 buck-boost 均衡 supervisory topology，不复用第三方未授权截图。它表达的是控制层如何根据 cell spread 分配均衡电流，而不是声称已经完成 MOSFET/PWM 级电路仿真。

![图 12 UPC 36-cell pack 实测 WLTP 工况](paper_figures/fig12_upc_measured_profile.png)

图 12 解读：UPC 实测数据给出 36 个 cell 的电压包络、支路电流和 BMS SOC。该图证明真实动态工况下 pack 内 cell voltage spread 可以达到数百 mV，包级均衡问题不是人为构造出来的玩具场景。

![图 13 UPC 实测 balancing semicycle](paper_figures/fig13_upc_real_balancing_semicycle.png)

图 13 解读：真实 BMS balancing semicycle 中 spread 会随工况切换上下波动，并不保证单调下降。该图用于提醒后续 Simulink/Python 均衡评估必须采用 paired initial condition，而不能只截取看起来下降的一小段。

![图 14 基于 UPC 高 spread 初值的 Python active buck-boost 短仿真](paper_figures/fig14_python_balancing_short_sim.png)

图 14 解读：在相同 UPC 高 spread 初值下，balancing off 基本保持原始 spread，而 active buck-boost 数字孪生在 30 min 内降低 46.30%。该结果说明 supervisory balancing law 有潜力改善一致性，但仍需电路级 Simulink/Simscape 验证开关损耗和器件约束。
...
图 15 解读：上排 SOC 曲线显示 W1 LSTM 在 20 Ah fresh/aged cell 上存在明显跨尺度误差；下排 SOH throughput 则能区分 fresh 与 aged，并给出接近 aged reference 的容量健康水平。因此这张图的价值是定位“迁移问题在哪里”，而不是证明 SOC zero-shot 已经可部署。
...
图 16 解读：左上为真实电站片段的大电流充放电切换，右上和左下分别是 8 个 cell 的电压/温度包络，右下是 W1 SOC 定性输出与 consistency proxy。由于该数据没有真值标签，不能计算 MAE/RMSE；当前能得出的结论是“数据接口和输出曲线能跑通且范围可解释”，不是“真实电站 SOC/SOH 精度已验证”。

![图 18 当前系统证据地图与泛化边界](paper_figures/fig18_results_dashboard.png)

图 18 解读：dashboard 把主结果压缩到四个 panel：单体快充、世界模型电压精度、多单体策略复制、跨数据集 SOC 泛化。绿色/蓝色部分是已达成的主链路证据，红色部分刻意保留 Zenodo zero-shot gap，帮助答辩时主动说明“哪些已验证、哪些仍是边界”。

## 6. 讨论

### 6.1 速度、安全与老化的协同

实验结果表明，SAC 策略并非简单增大充电电流，而是在世界模型预测和 ECM 安全投影下寻找更合适的动态电流轨迹。相比 CC-CV，SAC 在 paired episodes 中同时提升速度并降低 ΔSOH，说明 reward 中显式老化代理项对策略形态产生了有效约束。由于当前 Mamba 老化头未主导 ΔSOH，该结论应限定为“物理应力代理约束下的快充策略优化”，而非完整的数据驱动寿命模型优化。

### 6.2 物理安全层的重要性

纯数据驱动策略在训练分布外容易产生不安全动作。本文将 SOH-aware ECM 作为 L3 安全层，确保每一步动作在执行前都满足端电压约束，并用一阶 R0-SOH 修正让老化 cell 更保守。这样即使世界模型或策略网络出现局部误差，最终输出仍经过物理投影，适合向 BMS 工程场景解释。

### 6.3 包级扩展的边界

当前多单体原型是 Python independent-cell supervisory simulator，并未求解真实串联包 KCL/KVL，也未求解 PWM / MOSFET / buck-boost 开关电路。它可以说明单体策略复制和主动均衡协调的控制逻辑，但不能替代 Simulink / Simscape 的电路级验证。后续若使用已有 Simulink 30 模组资产，应先完成数据回放校准、策略闭环仿真和 balancing on/off paired test，再将其作为论文图或答辩演示。

![图 17 Python 包级策略与 Simulink 30 模组 / buck-boost 均衡资产的对接流程](paper_figures/fig17_simulink_pack_workflow.png)

图 17 解读：Python 侧负责输出 per-cell current、初始 SOC/V/T 和策略轨迹，Simulink 侧负责承接已有 pack plant 与 buck-boost balancer 做 balancing on/off paired test。这样可以把来源未知的 Simulink 资产降级为接口/电路烟测，避免替代 UPC 公开数据的定量结果。
...
2. Randomized raw 28-file 20-step rollout 未达标；温度 QC 后主指标为 26-file one-step 2.17 mV、20-step 24.29 mV。
3. SOH baseline 依赖容量字段，不代表无容量标签部署能力。
4. Mamba-head SOH 已完成轻量 ablation，但仍未达到 `<2% RMSE`，且尚未接入 external BatteryML trainer。
5. W4/W5 ΔSOH 当前 100% 由物理应力代理项主导，Mamba ΔSOH 老化头还没有形成有效独立贡献。
6. 世界模型 600-step replay 漂移较大：B0018 600-step V MAE / p95 为 170.56 / 550.20 mV，说明当前模型适合作短期滚动环境，不适合声称长时间开环 plant 替代。
7. Zenodo 6985321 zero-shot SOC 误差较高，当前只能说明跨尺度迁移瓶颈，不能说明已具备跨数据集高精度泛化。
8. Zenodo 18471156 无 SOC/SOH/容量标签，因此只能做真实电站定性展示。
9. 多单体主动均衡仍是 independent-cell supervisory / 数字孪生短仿真，不是串联包 KCL/KVL 或开关级电路仿真。

## 7. 结论

本文实现了一套面向 EV 智能快充决策的端到端原型系统。系统以 NASA PCoE 同源数据为训练主线，融合 SOC/SOH 状态估计、Mamba 世界模型、SAC 连续控制策略和 SOH-aware ECM 物理安全层。单体实验显示，SAC 策略相较 CC-CV 可在无过压的前提下显著提升充电速度，并在物理应力代理老化项约束下降低单循环 SOH 损耗。多单体实验进一步表明，单体策略可复制到多个 cell，并通过 SOC-spread 均衡协调改善一致性；但该结果仍属于 supervisory prototype，不替代真实串联包电路仿真。UPC 真实 36-cell pack 数据分析和 active buck-boost 短仿真为包级扩展提供了公开数据支撑。

后续工作将集中在三方面：其一，补齐 SOC 精度并针对 Randomized 高温/偏置电流组做 fine-tune；其二，将 Zenodo 6985321 / 18471156 从展示性实验推进到有标签或可校准的泛化评估；其三，将 Python 包级策略轨迹接入可信 Simulink / Simscape pack plant，完成电路级均衡验证。
...
| 补充图 19 | Randomized 全量 20-step rollout 复核 | 已生成 | `paper_figures/fig19_randomized_rollout_recheck.png` |
| 补充图 20 | B0018 长 horizon replay 漂移 | 已生成 | `paper_figures/fig20_closed_loop_replay.png` |
...
| 图 9 | 单体 SAC vs CC-CV vs MFCC I/V/SOC/T | 已生成 polished 版 | `paper_figures/fig09_charging_comparison_polished.png` |
| 图 10 | 6-cell 多单体策略复制对比 | 已生成 polished 版 | `paper_figures/fig10_pack_comparison_polished.png` |
...
| 图 18 | 当前系统证据地图 / results dashboard | 已生成 | `paper_figures/fig18_results_dashboard.png` |
| 图形摘要 | 系统流程 + 关键指标卡片 | 已生成 | `paper_figures/fig00_graphical_abstract.png` |
