...
需要注意，20-step open-loop 指标不能完全代表 600-step SAC 部署时的闭环误差。技术审评后，`BatteryChargingEnv._reset_history()` 已从完全重复的 64 步静态历史改为带微小物理噪声的 no-current 初始化，以降低 Mamba 初始输入的分布外程度；但 100/600-step 闭环电压误差仍需在 Mamba/WSL 环境下补充量化。因此本文把 20-step 结果作为短期动力学证据，把长 horizon 稳定性列为后续验证项。
...
6. 世界模型已报告 20-step open-loop 指标，但 100/600-step 闭环长 horizon 电压误差仍需补充量化。
