...
> 写作状态说明：本文按当前工程状态撰写，可作为本科生 AI 创新赛报告 / 论文初稿。文中结果仅使用已经跑通或已经生成的本地实验产物。SOC `<1.5%`、Zenodo 6985321 zero-shot、Zenodo 18471156 定性展示、BatteryML Mamba-head SOH 对比仍未完成，暂不作为已达成结论。
...
HUST 数据保留为可选展示，不进入当前训练管线。Zenodo 18471156 仅计划作为 W5 末尾真实电站定性展示，不参与训练或定量评估。
...
5. Zenodo 6985321 zero-shot 和 Zenodo 18471156 真实电站定性展示尚未完成。
6. 包级主动均衡仍是数字孪生短仿真，不是开关级电路仿真。
...
后续工作将集中在三方面：其一，补齐 SOC 精度与 Randomized 全量动态负载指标；其二，生成世界模型预测、ECM 安全投影、SAC reward 曲线和 zero-shot 泛化图；其三，将 Python 包级策略轨迹接入可信 Simulink / Simscape pack plant，完成电路级均衡验证。

## 8. 图表规划

| 图号 | 内容 | 当前状态 | 建议路径 |
|---|---|---|---|
| 图 1 | 三层系统总体架构图 | 缺失 | 待生成 |
| 图 2 | NASA / UPC 数据流与训练划分 | 缺失 | 待生成 |
| 图 3 | B0018 SOC 预测曲线 | 已有 | `outputs/figures/soc_b0018_prediction.png` |
| 图 4 | SOH 预测 vs 真实 / 容量退化曲线 | 缺失 | 待生成 |
| 图 5 | Mamba 世界模型预测 vs 真实 | 缺失 | 待生成 |
| 图 6 | ECM 安全动作投影示意 | 缺失 | 待生成 |
| 图 7 | SAC reward / training curve | 缺失 | 待生成 |
| 图 8 | 单体 SAC vs CC-CV vs MFCC I/V/SOC/T | 已有 | `outputs/eval_w4_final_default/charging_comparison.png` |
| 图 9 | 单体核心指标柱状图 | 缺失 | 待生成 |
| 图 10 | 6S1P pack 策略对比 | 已有 | `outputs/eval_pack_6s1p_h1200/pack_comparison.png` |
| 图 11 | UPC 实测 pack profile | 已有 | `outputs/upc_pack_paper/fig_upc_measured_profile.png` |
| 图 12 | UPC balancing semicycle | 已有 | `outputs/upc_pack_paper/fig_upc_real_balancing_semicycle.png` |
| 图 13 | active buck-boost 短仿真 | 已有 | `outputs/upc_pack_paper/fig_python_balancing_short_sim.png` |
| 图 14 | Zenodo 6985321 zero-shot SOC/SOH 曲线 | 缺失 | 待生成 |
| 图 15 | Zenodo 18471156 真实电站定性图 | 缺失 | 数据未下载 |
