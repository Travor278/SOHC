...
动态负载 Randomized 全量复核已完成。修复 `data.step` loader 后，`RW9/RW11/RW12` 也按统一 `stride=64` 缓存，最终覆盖 28/28 个 RW 文件。结果显示，PCoE 训练出的 residual Mamba 在 Randomized 全量上 one-step V MAE 为 10.07 mV，20-step rollout V MAE 为 103.43 mV、p95 为 763.48 mV；其中 `RW2/RW3` 是主要异常来源。去除 `RW2/RW3` 后，weighted one-step/20-step V MAE 分别为 2.26 mV 和 25.63 mV。因此，本文将 Randomized 全量结果作为动态负载压力边界和后续 Randomized fine-tune 动机，而不写成 `<10 mV` 达标结果。

![补充图 19 Randomized 全量 20-step rollout 复核](paper_figures/fig19_randomized_rollout_recheck.png)

补充图 19 解读：左图给出 28 个 RW 文件的逐文件 20-step 电压 MAE，红色标出 `RW2/RW3` 两个主导异常子集；右图对比全量 28 文件与去除 `RW2/RW3` 后的加权均值。该图主动揭示世界模型在动态负载域迁移下的边界，避免只展示顺滑子集。
...
2. Randomized 20-step 动态负载 rollout 已完成定稿复核，但 28-file strict full 未达标，需作为压力边界说明。
...
后续工作将集中在三方面：其一，补齐 SOC 精度并针对 `RW2/RW3` 做 Randomized fine-tune；其二，将 Zenodo 6985321 / 18471156 从展示性实验推进到有标签或可校准的泛化评估；其三，将 Python 包级策略轨迹接入可信 Simulink / Simscape pack plant，完成电路级均衡验证。
